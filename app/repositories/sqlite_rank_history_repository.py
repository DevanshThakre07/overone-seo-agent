"""SQLite store for keyword rank snapshots over time."""
...
class SqliteRankHistoryRepository:
    ...
    def record(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        checked_at = row.get("checked_at") or _utcnow().isoformat()
    ...
    def list_history(
        ...
        *,
        ...
        params: list[Any] = []
        if seed_url:
            clauses.append("seed_url = ?")
            params.append(normalize_url(seed_url))
        if target_domain:
            clauses.append("target_domain = ?")
            params.append(str(target_domain).lower().lstrip("www."))
        if keyword:
        ...
        sql = (
            ...
            f"{where} ORDER BY checked_at DESC LIMIT ?"
        )
        params.append(limit)
        with self._connect() as conn:
